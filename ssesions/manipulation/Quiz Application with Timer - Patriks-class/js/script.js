...
let timerInterval;
let points = 0;
let totalNumoNumofQuestions = questions.length;
...
let optionSelect = function (opt, q, o) {
    for (let i = 0; i <= questions[0].options.length; i++) {
        if (questions[q].options[i] == questions[q].answer) {
            option_list.childNodes[i].classList.add("correct");
            points++;
        }
        option_list.childNodes[i].classList.add("disabled");
    }
    ...
    total_que.innerHTML = `${activeQuestion}` + " / 6 questions";
    if (activeQuestion == totalNumoNumofQuestions) {
        quiz_box.classList.remove("activeQuiz");
        result_box.classList.add("activeResult");
        score_text.textContent = points + "from" + totalNumoNumofQuestions;
    }
